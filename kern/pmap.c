...
//            cprintf("%k0eWarning: ptp not exist for %p\n", va);
            ...
            if (new_page == NULL) {
                cprintf("%k04Fail to create: out of memory\n");
                return NULL;
            }
            new_page->pp_ref++;
				...
				physaddr_t pa, int perm)
{
    ...
    for (i = 0; i < npages; i++) {
        ...
        uint32_t cur_pa = pa + i * PGSIZE;
        pte_t *pte_ptr = pgdir_walk(pgdir, (void *) cur_va, true);
        *pte_ptr = cur_pa | perm | PTE_P;
    }
}
...
{
    // This function will force va alignment (offset ignored)
	pte_t *pte_ptr = pgdir_walk(pgdir, va, true);
    if (pte_ptr == NULL) {
        cprintf("%k04Fail to insert: out of memory\n");
        return -E_NO_MEM;
    }
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    if (pp == NULL) {
        cprintf("%0eWarning: nothing to remove at %p", va);
        return;
    }
    tlb_invalidate(pgdir, va);
    page_decref(pp);
...
{
	// LAB 3: Your code here.
	uint32_t org = (uintptr_t)va;

	if ((uintptr_t)va & 0xfff)
	{
		len += (uintptr_t)va & 0xfff;
	...
	for(uintptr_t i = 0; i < len; i += PGSIZE)
	{
		pte_t* pt = pgdir_walk(env->env_pgdir, va + i, 0);
		if (pt == NULL
            || (uintptr_t)va + i > ULIM
            || (perm & (uintptr_t) *pt) != perm)
		{
            user_mem_check_addr = (uintptr_t)va + i < org ? org : (uintptr_t)va + i;
			return -E_FAULT;
		}
	}
